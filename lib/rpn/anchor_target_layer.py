...
class AnchorTargetLayer(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        gt_boxes_all = bottom[1].data
        # im_info
        im_info = bottom[2].data[0, :]
        im_info_all = bottom[2].data

        if DEBUG:
            print ''
            print 'im_size: ({}, {})'.format(im_info[0], im_info[1])
            print 'scale: {}'.format(im_info[3])
        ...
        labels_orig = np.empty((len(inds_inside), ), dtype=np.float32)
        labels_orig.fill(-1)
        ...
        for img_index in xrange(batch_size):
            ...
            if DEBUG:
                print "-----------------------------anchor_target_layer:{},{}".format(nnn_fg, nnn_bg)
            bbox_targets = np.zeros((len(inds_inside), 4), dtype=np.float32)
            # print len(anchors)
            # print anchors
            # print len(argmax_overlaps)
            ...
            if cfg.TRAIN.RPN_POSITIVE_WEIGHT < 0:
                ...
                examples_count_total += num_examples
            else:
            ...
            top[0].data[img_index] = labels

            # bbox_targets
            bbox_targets = bbox_targets \
                .reshape((1, height, width, A * 4)).transpose(0, 3, 1, 2)
            top[1].data[img_index] = bbox_targets
            ...
            top[2].data[img_index] = bbox_inside_weights
            ...
            top[3].data[img_index] = bbox_outside_weights

        assert cfg.TRAIN.RPN_POSITIVE_WEIGHT < 0, \
           'Only RPN_POSITIVE_WEIGHT < 0 supported'
        # print "anchor_target_layer count:{}".format(examples_count_total)
        top[3].data[...] = top[3].data[...] / examples_count_total
