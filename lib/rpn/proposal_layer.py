...
class ProposalLayer(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        for img_index in xrange(batch_size):
            ...
            bbox_deltas = bottom[1].data[img_index:img_index+1]
            ...
            scores = bottom[0].data[img_index:img_index+1, self._num_anchors:, :, :]
            ...
            batch_inds[:, 0] = img_index
            blob = np.hstack((batch_inds, proposals.astype(np.float32, copy=False)))

            if blobs is None:
                blobs = blob
            else:
                blobs = np.vstack((blobs, blob))

        top[0].reshape(*(blobs.shape))
        top[0].data[...] = blobs
