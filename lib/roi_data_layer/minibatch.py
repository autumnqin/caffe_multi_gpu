...
def get_minibatch(roidb, num_classes):
    ...
    if cfg.TRAIN.HAS_RPN:
        ...
        gt_boxes = np.empty((num_images, len(gt_inds), 5), dtype=np.float32)
        ...
        gt_count_max = 0
        gt_boxes_all = []
        blobs['im_info'] = np.empty((num_images, 7), dtype=np.float32)
        for i in xrange(num_images):
            gt_inds = np.where(roidb[i]['gt_classes'] != 0)[0]
            gt_count = len(gt_inds)
            gt_boxes = np.empty((gt_count, 5), dtype=np.float32)
            gt_boxes[:, 0:4] = roidb[i]['boxes'][gt_inds, :] * im_scales[i]
            gt_boxes[:, 4] = roidb[i]['gt_classes'][gt_inds]
            gt_boxes_all.append(gt_boxes)
            blobs['im_info'][i, :] = np.hstack((im_blob.shape[2], im_blob.shape[3], gt_count, im_scales[i]))
            if gt_count > gt_count_max:
                gt_count_max = gt_count
