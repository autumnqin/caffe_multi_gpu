...
#def _demoImage(net, net1, ff, im, _t, vis=True, thresh=0.01, max_per_image=100):
def _demoImage(net, net1, im, _t, vis=True, thresh=0.01, max_per_image=100):
    # Detect all object classes and regress object bounds
    timer = Timer()
    timer.tic()
    scores, boxes = im_detect(net, im, _t, None)
    timer.toc()
    ...
    retstr = ""
    num_classes = 2
    all_boxes =  [_ for _ in xrange(num_classes)]
    ...
    person = 1
    for box in all_boxes[person:person+1]:
        max_area = im.shape[0] * im.shape[1] / 100
        max_str = None
        for i in xrange(np.minimum(10, box.shape[0])):
            bbox = box[i, :4]
            score = box[i, -1]
            if(score > 0.1):
                retstr += " "
                retstr += str(score)
                retstr += ","
                retstr += str(int(bbox[0]))
                retstr += ","
                retstr += str(int(bbox[1]))
                retstr += ","
                retstr += str(int(bbox[2]))
                retstr += ","
                retstr += str(int(bbox[3]))
            if score > 0.8:
                cv2.rectangle(im, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (0, 256, 0), 1)

                area = (bbox[2] - bbox[0]) * (bbox[3] - bbox[1])
                #if area > max_area:
                #    im_fet = im[int(bbox[1]):int(bbox[3]), int(bbox[0]):int(bbox[2])]
                #    max_str = im_feature(net1, im_fet, ff, _t)
                #    max_area = area
...
def _mark_video(net, net1, video, gt, _t, max_per_image=100):
    ...
    if capture.isOpened():
        ...
        count=0;
        #f1 = open(gt,'w')
        #ff = open("/home/qinqiuping/ml/data/5",'w')
        while True:
            ...
            if ret == True:
               ...
               #     continue;
                ...
                #retstr = _demoImage(net, net1, ff, prev, _t, max_per_image=100)
                retstr = _demoImage(net, net1, prev, _t, max_per_image=100)

                #f1.write(str(count) + retstr)
                #f1.write("\n")
                count = count + 1
            else:
                break

def mark_gt(net, net1, video_dir, mark_dir, max_per_image=100, thresh=0.01, vis=False):
    _t = {'im_preproc': Timer(), 'im_net' : Timer(), 'im_postproc': Timer(), 'misc' : Timer()}
    for item in os.listdir(video_dir):
        # if item != "video_16.mp4":
        #     continue
        gt_file = os.path.join(mark_dir, item + ".txt")
        video = os.path.join(video_dir, item)
        _mark_video(net, net1, video, gt_file, _t)



def im_feature(net, im, ff, _t):
    blobs = _get_blobs1(im)

    if cfg.TEST.HAS_RPN:
        im_blob = blobs['data']
        blobs['im_info'] = np.array(
            [np.hstack((im_blob.shape[2], im_blob.shape[3], 1))],
            dtype=np.float32)
    ...
    feature = net.blobs['inc4e'].data[0].flatten()

    strs = ""
    for index in xrange(len(feature)):
        strs += "   "
        strs += str(feature[index])
    print strs
...
def _get_blobs1(im):
    """Convert an image and RoIs within that image into network inputs."""
    blobs = {'data' : None, 'rois' : None}
    blobs['data'] = _get_image_blob1(im)
    return blobs

def _get_image_blob1(im):
    im_orig = im.astype(np.float32, copy=True)
    im_orig -= cfg.PIXEL_MEANS
    processed_ims = []
